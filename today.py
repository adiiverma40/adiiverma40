...
def recursive_loc(
    ...
    if request.status_code == 200:
        if (request.json()["data"]["repository"]["defaultBranchRef"]
                != None):  # Only count commits if repo isn't empty
            return loc_counter_one_repo(
                owner,
                repo_name,
                data,
                cache_comment,
                request.json()["data"]["repository"]["defaultBranchRef"]
                ["target"]["history"],
                addition_total,
                deletion_total,
                my_commits,
...
def loc_query(owner_affiliation,
    ...
    if request.json()["data"]["user"]["repositories"]["pageInfo"][
            "hasNextPage"]:  # If repository data has another page
        edges += request.json()["data"]["user"]["repositories"][
            "edges"]  # Add on to the LoC count
        return loc_query(
            owner_affiliation,
            comment_size,
            force_cache,
            request.json()["data"]["user"]["repositories"]["pageInfo"]
            ["endCursor"],
            edges,
        )
    else:
...
def cache_builder(edges, comment_size, force_cache, loc_add=0, loc_del=0):
    ...
    for index in range(len(edges)):
        ...
        if (repo_hash == hashlib.sha256(
                edges[index]["node"]["nameWithOwner"].encode(
                    "utf-8")).hexdigest()):
            try:
                if (int(commit_count) != edges[index]["node"]
                    ...
                    data[index] = (repo_hash + " " +
                                   str(edges[index]["node"]["defaultBranchRef"]
...
def flush_cache(edges, filename, comment_size):
    ...
    with open(filename, "w") as f:
        ...
        for node in edges:
            f.write(
                hashlib.sha256(node["node"]["nameWithOwner"].encode(
...
def stars_counter(data):
    ...
    for edge in data:
        try:
            # Attempt to safely add the stargazerCount
            total_stars += edge["node"]["stargazerCount"]
        except (TypeError, KeyError):
            # If edge, "node", or "stargazerCount" is None or missing, skip it
            print(edge)
            continue
...
def formatter(query_type, difference, funct_return=False, whitespace=0):
    ...
    print("{:>12}".format("%.4f" % difference +
                          " s ")) if difference > 1 else print(
                              "{:>12}".format("%.4f" % (difference * 1000) +
                                              " ms"))
    if whitespace:
...
if __name__ == "__main__":
    ...
    for index in range(len(total_loc) - 1):
        total_loc[index] = "{:,}".format(
            total_loc[index])  # format added, deleted, and total LOC
